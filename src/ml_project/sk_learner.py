...
#     test = test.drop("original_gene_symbol",axis=1)
    ...
    for row in range(0,samples):
        ...
        info = [chromosome,start,end]
        gene_locations.append(info)
    test['locations'] = gene_locations
    
    ###################################
    ##### Find Nearest Mutation     ##
    ##################################
    mutations = pickle.load("patient_mutations.obj","wb")
    new_mutations = []
    for row in range(0,samples):
        id = train.iloc[row,0]
        info = train.iloc[row,10]
        chromosome = info[0]
        start = info[1]
        end = info[2]
        nearest = get_nearest(id,chromosome,start,end,mutations)
...
def get_actual_location(relative,table):
    ...
    try:
       ...
       copy = [out[0],out[1],out[2]]
    except:
        print relative
        print 'failed'
        return ['','','']
    return copy
def get_nearest(id,chromosome,start,end,mutations):
    mid = np.ndarray([(start + end) / 2.0] * len(mutations[id][chromosome]))
    list_mutations = np.ndarray(sorted(mutations[id][chromosome]))
